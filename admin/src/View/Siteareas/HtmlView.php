...
namespace NPEU\Component\Siteareas\Administrator\View\Siteareas;
...
class HtmlView extends BaseHtmlView {
    ...
    */
    ...
    /**
     * Display the main "Siteareas" view
     *
    ...
    function display($tpl = null) {
        ...
        $this->canDo = ContentHelper::getActions('com_siteareas');
    ...
    {
        $title = Text::_('COM_SITEAREAS_MANAGER_RECORDS');
        ...
        if ($this->canDo->get('core.create')) {
            ToolBarHelper::addNew('sitearea.add', 'JTOOLBAR_NEW');
        }
        if ($this->canDo->get('core.edit')) {
            ToolBarHelper::editList('sitearea.edit', 'JTOOLBAR_EDIT');
        }


        if (!$this->isEmptyState && ($this->canDo->get('core.edit.state'))) {
            if ($this->canDo->get('core.delete')) {
                //ToolBarHelper::deleteList('', 'siteareas.delete', 'JTOOLBAR_DELETE');
                if ($this->state->get('filter.published') != -2) {
                    ToolBarHelper::deleteList('', 'siteareas.trash', 'JTOOLBAR_DELETE');
                }
            }

            if ($this->canDo->get('core.edit') || Factory::getApplication()->getIdentity()->authorise('core.manage', 'com_checkin')) {
                ToolBarHelper::checkin('siteareas.checkin');
            }
        ...
        if (!$this->isEmptyState && $this->state->get('filter.published') == -2 && $this->canDo->get('core.delete')) {
            ToolBarHelper::deleteList('', 'siteareas.delete', 'JTOOLBAR_EMPTY_TRASH');
            /*$toolbar->delete('siteareas.delete', 'JTOOLBAR_EMPTY_TRASH')
        ...
        if ($this->canDo->get('core.admin')) {
            ...
            ToolBarHelper::preferences('com_siteareas');
        }
    }
}